...
clear
echo -e "${BLUE}=======================================${NC}"
echo -e "${YELLOW}     Han Installer Tools Panel       ${NC}"
echo -e "${BLUE}=======================================${NC}"
...
add_database_admin() {
    ...
    mysql -u root -p"$root_pass" -e "CREATE USER '$db_user'@'admin' IDENTIFIED BY '$db_pass';"
    mysql -u root -p"$root_pass" -e "GRANT ALL PRIVILEGES ON *.* TO '$db_user'@'admin' WITH GRANT OPTION;"
